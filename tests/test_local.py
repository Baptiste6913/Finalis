...
env=dict(os.environ, PRESCREEN_MOCK='1', PRESCREEN_PORT='8790', PRESCREEN_QUIET='1', PRESCREEN_DATA=TMPDATA)
...
for _ in range(50):
    try:
        h=json.loads(urllib.request.urlopen('http://127.0.0.1:8790/api/health').read()); print('HEALTH', h); break
...
async def main():
    async with async_playwright() as pw:
        ...
        q=json.loads(urllib.request.urlopen('http://127.0.0.1:8790/api/db/query?collection=submissions').read())
        print('SERVER SUBMISSIONS', len(q['docs']), 'assetId', q['docs'][0]['data']['file'].get('assetId'))
        checks = {'stored_on_server': len(q['docs']) == 1 and bool(q['docs'][0]['data']['file'].get('assetId'))}
        ...
        checks['reviewer_opens_pdf'] = await page.evaluate('UI.S.doc.kind') == 'pdf' and await page.evaluate('UI.S.doc.pages.length') == 28
        await page.click('[data-card="f1"] .verdict button.yes'); await page.wait_for_timeout(500)
        exp=json.loads(urllib.request.urlopen('http://127.0.0.1:8790/api/learning/export').read())
        print('EXPORT verdicts', len(exp['verdicts']), 'rules', len(exp['learned_rules']), 'submissions', exp['submissions'])
        checks['verdict_on_server'] = len(exp['verdicts']) == 1 and exp['submissions'] == 1
        ...
        await page.screenshot(path=os.path.join(OUT, 'shot_local.png'), full_page=True)
        await browser.close()
        bad = [k for k, v in checks.items() if not v]
        print('ALL OK' if not bad else 'FAILURES: ' + ', '.join(bad))
        if bad: srv.terminate(); sys.exit(1)
