...
env=dict(os.environ, PRESCREEN_MOCK='1', PRESCREEN_PORT='8789', PRESCREEN_QUIET='1')
...
async def main():
    async with async_playwright() as pw:
        ...
        d = await dl.value
        path = '/tmp/dl_banker.pdf'; await d.save_as(path)
        print('download:', d.suggested_filename, os.path.getsize(path), 'bytes')
        await page.screenshot(path='shot_summary_pdfbtn.png')
        ...
        d3 = await dl3.value; p3='/tmp/dl_desk.pdf'; await d3.save_as(p3); print('desk download:', d3.suggested_filename, os.path.getsize(p3), 'bytes')
        ...
        print('brief button:', await page.evaluate("!!Array.from(document.querySelectorAll('#railbody button')).find(b=>b.textContent.includes('Export the full brief as PDF'))"))
        await browser.close()
